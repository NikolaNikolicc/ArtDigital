<?php
	...
	// sendConfirmation('3d39d790-7ab0-4e36-bf6e-59b3029aec20');
	// die();

	$db = new DB();
	$db->query("SELECT data, session, id FROM slike WHERE id = 4962");
	...
	if ($db->getRowsNum() > 0) {
		...
		wrap($data, $images, $db, $data[2]);
	}

	function wrap($data, $images, $db, $order_id) {
		error_reporting(E_ALL);
		ini_set('display_errors',1);
